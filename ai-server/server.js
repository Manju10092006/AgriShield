...
// ─── Gemini Initialization (per spec) ─────────────────────────────
...
app.post('/ai/chat', async (req, res) => {
  ...
  const contextText = context
    ? `
Context:
Crop: ${context.plant || 'Unknown'}
Detected disease: ${context.disease || 'Unknown'}
  ...
  try {
    const chat = model.startChat({
      history: [
        {
          role: 'user',
          parts: [{ text: 'You are a helpful agricultural assistant chatbot.' }]
        }
      ]
    });

    const userMessage = `
IMPORTANT RULE: You must respond ONLY in ${langLabel}. Do not mix other languages. Every word must be in ${langLabel}.
...
Respond as an expert agronomist. Keep answers concise, practical, and under 120 words.
Ensure the language strictly follows ${langLabel}.
`.trim();
...
app.get('/ai/health', (_req, res) => {
  ...
  return res.json({ status: 'ok', model: 'gemini-2.5-flash' });
});
